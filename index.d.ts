...
declare module wx {
    ...
    export module types {
        ...
        type SocketCloseParams = {
            /**
             * 一个数字值表示关闭连接的状态号，表示连接被关闭的原因。如果这个参数没有被指定，默认的取值是 1000 （表示正常连接关闭）
             */
            code?: number,
            /**
...
/**
 * IOS 及安卓不支持：
 *     globalCompositeOperation 不支持以下值：source-in source-out destination-atop lighter copy
 *     isPointInPath 不支持
  */
 ...
 *     compressedTexImage2D 不支持
 *     compressedTexSubImage2D 不支持
 * 
 * 安卓不支持：
 *     getExtension
...
declare class WxWebGLRenderingContext extends WebGLRenderingContext {
    /**
     * 将一个 Canvas 对应的 Texture 绑定到 WebGL 上下文。(目前该方法仅支持 iOS 6.6.6 以上版本，Android / 开发者工具暂不支持。)
...
declare class RecorderManager {
    ...
    public start(param: {
        /**
         * 录音的时长，单位 ms，最大值 600000（10 分钟），默认值 60000（1 分钟）
         */
        duration?: number,
        /**
         * 采样率
        ...
        frameSize: number,
        /**
         * 指定录音的音频源，可通过 wx.getAvailableAudioSources() 获取当前可用的音频源，默认值 auto
         */
...
declare class Video {
    ...
    /**
     * 视频的初始播放位置，单位为 s 秒，默认值 0
     */
    public initialTime: number;
    /**
     * 视频的播放速率，有效值有 0.5、0.8、1.0、1.25、1.5 默认值 1.0
     */
...
declare module wx {
    ...
    * 监听开始始触摸事件
    ...
    //-- 加速计
    /**
    * 监听加速度数据，频率：5 次 / 秒，接口调用后会自动开始监听，可使用 wx.stopAccelerometer 停止监听。
    */
    ...
    * 拉取当前用户所有同玩好友的托管数据。该接口只可在开放数据域下使用
     ...
     * 托管数据的限制
           > 每个 openid 所标识的微信用户在每个游戏上托管的数据不能超过 128 个 key-value 对。
    ...
    * 根据用户当天游戏时间判断用户是否需要休息
    ...
    /**
     * @deprecated 该接口已过期，微信不允许主动弹出授权框，该 API 会被逐渐作废，请使用 wx.createUserInfoButton 和在隔离数据区取得用户信息
    ...
    export function getUserInfo(param: {
        ...
        success?: (res: {
            ...
            rawData: string,
            /**
             * 使用 sha1(rawData + sessionkey) 得到字符串，用于校验用户信息，参考文档 signature(https://mp.weixin.qq.com/debug/wxagame/dev/tutorial/open-ability/http-signature.html?t=201822)
             */
            signature: string,
            /**
    ...
    * 发起米大师支付
    ...
    export function requestMidasPayment(param: {
         ...
         * mode 为 game（购买游戏币）时，buyQuantity 不可任意填写。需满足 buyQuantity * 游戏币单价 = 限定的价格等级。如：游戏币单价为 0.1 元，一次购买最少数量是 10。
         * 有效价格等级如下：
                价格等级（单位：人民币）
                1
                3
